...
export class MovieService {
  ...
  getPopular(category: string, page?: any) {
    ...
    switch(true) {
      case !category:
        ...
        break;
      case category === 'G': 
        url += `?certification=${category}&sort_by=popularity.desc`;
        break;
      case category === 'now': 
        url += `?primary_release_date.gte=2019-12-01&primary_release_date.lte=2019-12-15`;
        break;
      case category === '2019' || category === '2018' || category === '2017' || category === '2016' || category === '2015' || category === '2014':
  ...
  getYear(year: string) {
    return this.http.get<any>(`${environment.api}/?primary_release_year=${year}&sort_by=vote_average.desc`);
  }
